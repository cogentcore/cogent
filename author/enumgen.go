...
var _FormatsValues = []Formats{0, 1, 2, 3}

// FormatsN is the highest valid value for type Formats, plus one.
const FormatsN Formats = 4

var _FormatsValueMap = map[string]Formats{`pdf`: 0, `html`: 1, `docx`: 2, `epub`: 3}

var _FormatsDescMap = map[Formats]string{0: `PDF, via LaTeX, with full math support.`, 1: `HTML is a single standalone .html file.`, 2: `DOCX is a Microsoft Word compatible .docx file.`, 3: `EPUB is a standard eBook .epub file.`}

var _FormatsMap = map[Formats]string{0: `pdf`, 1: `html`, 2: `docx`, 3: `epub`}
