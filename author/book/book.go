...
func (bk *BookData) PDF(mdfn string) error {
	...
	// todo: generate these filter files from embeds
	shell.Run("pandoc", "-f", mdopts, "-F", "../author/pandoc-filter.py", "--lua-filter", "../author/glossary-filter.lua")
	return nil
}
...
func (bk *BookData) HTML(mdfn string) error {
	...
	trg := bk.Name + ".html"
	// todo: generate these filter files from embeds
	shell.Run("pandoc", "-f", mdopts, "-F", "../author/pandoc-filter.py", "--lua-filter", "../author/glossary-filter.lua", "-t", "html", "--standalone", "--embed-resources", "--number-sections", "--css=../author/html.css", "-H", "../author/head_include.html", "-o", trg, mdfn)
	return nil
}

// Markdown generates the combined markdown file that everything else works on.
func (bk *BookData) Markdown() string {
	shell.RunErrOK("mkdir", "author")
	fn := "author/book.md"
	bk.GetFiles()
	bk.Metadata(fn)
