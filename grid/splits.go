...
type Split struct {
	...
	Splits []float32 `min:"0" max:"1" step:".05" fixed-len:"3"`
}
...
func (lt *Splits) SplitByName(name SplitName) (*Split, int, bool) {
	...
	fmt.Printf("grid.SplitByName: split named: %v not found\n", name)
	return nil, -1, false
}
...
func (lt *Splits) FixLen() {
	for _, sp := range *lt {
		ln := len(sp.Splits)
		if ln > 3 {
			sp.Splits = sp.Splits[:3]
		} else if ln < 3 {
			sp.Splits = append(sp.Splits, make([]float32, 3-ln)...)
		}
	}
}
